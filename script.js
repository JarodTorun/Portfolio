...
function buildCatalogCard(row) {
  ...
  card.href = slug === 'id' ? 'CV/index.html' : `projets/index.html?projet=${encodeURIComponent(slug)}`;
...
// Catalogue mobile (voir #catalog dans index.html) : une carte par projet,
...
if (skipIntro) {
  ...
  // Vidéo de fond floutée du splash (voir #overlay-video dans index.html,
  ...
  if (!isSlowConnection()) {
    // Même logique que /projets/ (setupHero, voir le commentaire détaillé
    // là-bas) : on attend "canplaythrough" dans une fenêtre courte avant de
    // s'engager sur la vidéo (jamais lancée avant, voir pas d'autoplay dans
    // index.html) - si ça n'arrive pas à temps, on abandonne SANS avoir
    // jamais montré le moindre frame. Pas d'image de repli dédiée ici
    // (contrairement au hero de /projets/) : #overlay garde simplement son
    // fond noir uni (voir style.css), déjà cohérent sans elle.
    // Plus généreux sur mobile (voir /projets/ script.js, setupHero) : le
    // pipeline vidéo de Safari iOS met plus de temps à s'initialiser.
    ...
    // loadeddata, pas canplay/canplaythrough (voir /projets/ script.js,
    // setupHero, pour le détail) : ni l'un ni l'autre ne se déclenche de
    // façon fiable sur Safari iOS (testé sur un vrai iPhone) - la vidéo
    // tombait donc systématiquement sur le timeout et ne se lançait jamais.
    // loadeddata (1re frame décodée) est l'étape la plus précoce dispo.
