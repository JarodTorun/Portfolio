...
if (skipIntro) {
  ...
  // vers son parent.
  //
  // pointermove peut se déclencher bien plus vite que l'écran ne rafraîchit :
  // écrire ces variables (et donc redessiner le dégradé) à chaque événement
  // saturait le thread principal et faisait décrocher la vidéo de fond. On ne
  // garde que la dernière position connue et on applique au plus une fois par
  // frame via requestAnimationFrame.
  let lastGlowEvent = null;
  let glowFramePending = false;

  var applyButtonGlow = function () {
    glowFramePending = false;
    if (!lastGlowEvent) return;
    powerSwitchGlow.style.setProperty('--x', lastGlowEvent.clientX.toFixed(2));
    powerSwitchGlow.style.setProperty('--y', lastGlowEvent.clientY.toFixed(2));
    powerSwitchGlow.style.setProperty('--xp', (lastGlowEvent.clientX / window.innerWidth).toFixed(2));
  };

  var syncButtonGlow = function (e) {
    lastGlowEvent = e;
    if (glowFramePending) return;
    glowFramePending = true;
    requestAnimationFrame(applyButtonGlow);
  };
  ...
  // Même throttling qu'au-dessus, et on ne mesure plus getBoundingClientRect()
  // (qui force un reflow synchrone) qu'une fois à l'entrée du survol, plus à
  // chaque mousemove : la taille/position du bouton ne change pas pendant
  // qu'on le survole.
  let tiltRect = null;
  let lastTiltEvent = null;
  let tiltFramePending = false;

  var applyButtonTilt = function () {
    tiltFramePending = false;
    if (!lastTiltEvent || !tiltRect) return;
    const relX = (lastTiltEvent.clientX - tiltRect.left) / tiltRect.width - 0.5; // -0.5 .. 0.5
    const relY = (lastTiltEvent.clientY - tiltRect.top) / tiltRect.height - 0.5;
  ...
  var tiltButton = function (e) {
    lastTiltEvent = e;
    if (tiltFramePending) return;
    tiltFramePending = true;
    requestAnimationFrame(applyButtonTilt);
  };

  var resetButtonTilt = function () {
    tiltRect = null;
