...
// même principe que /projects/ (script.js) : figé au chargement, pas
...
// Identifiant de projet partagé avec /projects/ : le nom officiel en
...
// URL propre par projet (demande explicite) : /projects/<dossier>/, chaque
// dossier une copie du template avec <base href="../"> (voir FOLDER_SLUG
// dans /projects/script.js pour le sens inverse) - AVEC le slash final, SANS
...
// "montés" (sans animation) au retour sur l'accueil depuis /projects/. En
...
function buildScene(rows) {
  ...
  rows.forEach(row => {
    ...
    // "ID" n'utilise pas le gabarit /projects/, il pointe vers /cv/ (gabarit à
    ...
    hoverZone.addEventListener('click', () => {
      ...
      const destination = slug === 'id'
        ? 'cv/'
        : `projects/${PROJECT_URL_SLUG[slug] || slug}/`;
...
// Rempli par buildCatalog, consommé par le pageshow plus bas (bfcache) :
// même principe que builtEntries pour la scène desktop - permet de remettre
// à jour l'image monté/démonté des cartes déjà construites sans reconstruire
// tout le catalogue.
const builtCatalogEntries = [];

// Image (fond) d'une carte catalogue pour un état donné - extrait de
// buildCatalogCard pour être réutilisable au retour depuis le bfcache (voir
// pageshow plus bas), pas seulement à la construction initiale.
function applyCatalogCardImage(row, image, isVisited) {
  const slug = getSlug(row);
...
function buildCatalogCard(row) {
  ...
  card.href = slug === 'id' ? 'cv/' : `projects/${PROJECT_URL_SLUG[slug] || slug}/`;
  ...
  applyCatalogCardImage(row, image, isVisited);
  ...
  builtCatalogEntries.push({ row, image });
...
// navigation vers /projects/, sans que le moindre script ne rejoue. Elle
...
window.addEventListener('pageshow', (event) => {
  ...
  // Catalogue mobile (voir builtCatalogEntries/buildCatalogCard) : même
  // resynchronisation que builtEntries ci-dessus pour la scène desktop -
  // sans ça, un retour au bfcache sur mobile réaffichait toujours l'image
  // "démonté" d'origine, jamais mise à jour après la visite (signalé).
  builtCatalogEntries.forEach(({ row, image }) => {
    applyCatalogCardImage(row, image, visited.has(getSlug(row)));
  });
});
...
// /projects/) via la Navigation Timing API, seule façon fiable de savoir
...
// "Retour" depuis /projects/) -> on saute direct à la scène, sans rejouer
...
// /projects/ (setupHero), dupliquée ici faute de module partagé entre les
...
if (skipIntro) {
  ...
  if (!isSlowConnection()) {
    // Diagnostiqué sur un vrai iPhone (voir /projects/ script.js, setupHero,
...
if (skipIntro) {
  ...
  const ignite = () => {
    ...
    // (retour depuis /projects/ notamment, voir plus haut).
